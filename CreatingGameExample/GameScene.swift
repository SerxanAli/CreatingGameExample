...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        leftmBorder.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: 1, height: frame.height))
        leftmBorder.physicsBody?.affectedByGravity = false
        leftmBorder.physicsBody?.isDynamic = false
        leftmBorder.position = .init(x:-frame.width / 2, y: 0)
        addChild(leftmBorder)
